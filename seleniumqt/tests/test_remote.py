    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while conn:
            ...
            for command in commands:
                ...
                logger.debug(f"Message recieved from remote: {message=} {command=}")
            commands = []
...
def start_test_server():
    ...
    threading.Thread(target=test_server, args=(free_sock,), daemon=True).start()
    return free_sock
...
class TestRemote(unittest.TestCase):
    def test_init_1(self):
        ...
        logger.debug(f"{remote_proc.pid=}, {remote_proc.is_alive()=}, {remote_proc.name=}")
