...
class Remote(_QtWebEngineWidgets.QWebEngineView):
    ...
    def run_js(self, script_file_name: str) -> None:
        ...
        def return_callback(result: str):
            result = str(result) # convert the result to str if it is given in some other format.

            if result.startswith("JavascriptException"):
                logger.exception("This Here.")
                raise JavascriptException("There was a problem with the javascript", result, str(self.__console))
        ...
        self.page().runJavaScript(self.JAVASCRIPT_EXECUTION_SHELL.format(script=script), resultCallback=return_callback)
    ...
    def __recurrent(self) -> None:
        ...
        if (self.ready) and (self.command != ""):
            if self.STR_TO_COMMAND[self.command[:self.COMMAND_RESERVED_LENGTH]][1](self.command[self.COMMAND_RESERVED_LENGTH:]):
    ...
    def __console_recurrent(self) -> None:
        self.page().runJavaScript(self.CONSOLE_JAVASCRIPT, resultCallback=self.__update_console)
    ...
    def __get_data(self, key: str, required: bool = False) -> _typing.Any | _typing.Literal[False] | _typing.NoReturn:
    ...
    def __init__(self, data: dict) -> None:
        ...
        self.conn: _socket.socket = _socket.socket(_socket.AF_INET, _socket.SOCK_STREAM)
        ...
        self.__remote_client_thread = _threading.Thread(target=self.remote_client, daemon=True)
